...
# # read the file
# text_file = open("p083_matrix.txt", "r")
...
"""
We can take any path through the matrix now, by moving up, right left and down. 
We need the minimal path sum.
There are infinite number of paths, but anything that loops back on itself will not be minimal and does not have to be considered. 

We must start at the top left.
We must end at the bottom right.

Instead of holding just one layer, we need to maintain the currently most minimum path sum to reach every single space
